...
static int calc_vib_disp(channel_state *c, int phase) nothrow {
...
// do a Ex/Fx code
static void do_command(song_state *st, channel_state *c) nothrow {
	ubyte* p = c.ptr;
...
// $0654 + $08D4-$8EF
static void do_note(song_state *st, channel_state *c, int note) nothrow {
...
static void CF7(channel_state *c) nothrow {
	if (c.note_release)
		c.note_release--;
...
static int sub_cycle_calc(song_state *st, int delta) nothrow {
	if (delta < 0x8000)
