...
const(char)* internal_validate_track(BYTE *data, int size, BOOL is_sub) nothrow {
...
int compile_song(Song *s) nothrow {
	...
	// Put order
	WORD *wout = cast(WORD *)&spc[s.address];
	int first_pat = s.address + s.order_length*2 + (s.repeat ? 6 : 2);
	for (i = 0; i < s.order_length; i++)
		*wout++ = cast(WORD)(first_pat + (s.order[i] << 4));
	if (s.repeat) {
		*wout++ = cast(WORD)s.repeat;
		*wout++ = cast(WORD)(s.address + s.repeat_pos*2);
	}
	*wout++ = 0;
	...
	*tout++ = 0;
	...
	WORD *sub_table = cast(WORD*)calloc(WORD.sizeof, s.subs);
	for (BYTE *pos = tracks_start; pos < tracks_end; pos = next_code(pos)) {
		if (*pos == 0xEF) {
			int sub = *cast(WORD *)(pos + 1);
			if (sub >= s.subs) abort(); // can't happen

			if (sub_table[sub] == 0) {
				track *t = &s.sub[sub];
				sub_table[sub] = cast(WORD)(tout - &spc[0]);
				memcpy(tout, t.track, t.size + 1);
				tout += t.size + 1;
			}
			*cast(WORD *)(pos + 1) = sub_table[sub];
		}
	}
	free(sub_table);
