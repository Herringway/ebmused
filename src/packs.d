...
block *save_cur_song_to_pack() nothrow {
	block *b = get_cur_block();
	if (cur_song.changed) {
		int size = compile_song(&cur_song);
		b.size = cast(ushort)size;
...
int calc_pack_size(pack *p) nothrow {
	int size = 2;
	for (int i = 0; i < p.block_count; i++)
...
void new_block(block *b) nothrow {
	pack *p = &inmem_packs[packs_loaded[2]];
	int pos = 0;
	while (pos < p.block_count && p.blocks[pos].spc_address <= b.spc_address)
		pos++;

	block *newb = cast(block*)array_insert(cast(void**)&p.blocks, &p.block_count, block.sizeof, pos);
	*newb = *b;
	p.status |= IPACK_CHANGED;
	select_block(pos);
}

void delete_block(int block) nothrow {
	pack *p = &inmem_packs[packs_loaded[2]];
	select_block(-1);
	memmove(&p.blocks[block], &p.blocks[block+1], (--p.block_count - block) * block.sizeof);
	p.status |= IPACK_CHANGED;
}
...
void move_block(int to) nothrow {
	...
	if (to > from) {
		...
		memmove(&p.blocks[to + 1], &p.blocks[to], (from - to) * block.sizeof);
	}
	p.blocks[to] = b;
	current_block = to;
	p.status |= IPACK_CHANGED;
...
BOOL save_pack(int pack_) nothrow {
	...
	if (conflict != AREA_FREE) {
		char *p2 = &error[0];
		p2 += sprintf(p2, "Pack %02X could not be saved:\n", pack_);
		if (conflict == AREA_NOT_IN_FILE)
			strcpy(p2, "The ROM address is invalid");
	...
	free(rp.blocks);
	rp.blocks = cast(block*)memcpy(malloc(block.sizeof * p.block_count),
		p.blocks, block.sizeof * p.block_count);

	change_range(old_start, old_start + old_size, pack_, AREA_FREE);
	change_range(p.start_address, p.start_address + size, AREA_FREE, pack_);
